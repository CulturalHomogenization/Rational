...
@export var station_name: String = "Pressing Station"
@export var press_time: float = 3.0

# Simple item processing dictionary: input_item -> processed_scene
@export var item_processing: Dictionary = {
	"tomato": preload("res://Scenes/ChoppedCarrot.tscn"),
	#"salad": preload("res://Scenes/ChoppedOnion.tscn"),
	#"steamed veg": preload("res://Scenes/ChoppedPotato.tscn"),
}
...
func _ready() -> void:
	...
	processing_timer.wait_time = press_time
...
func start_processing() -> void:
	if processing_queue.size() > 0 and not is_processing:
		...
		interaction_actions = {
			"Processing": {
				"message": "Pressing " + processing_queue[0] + "...",
				"input_action": ""
			}
		}
		
		# Start animations if available
		if animation_player and animation_player.has_animation("pressing"):
			animation_player.play("pressing")
		
		if info:
			info.show_message("Started pressing " + processing_queue[0])
		
		print("Pressing started for: ", processing_queue[0])
...
func _on_processing_complete() -> void:
	if processing_queue.size() > 0:
		...
		if info:
			info.show_message("Pressing finished!")
...
func get_processing_status() -> String:
	if is_processing:
		...
		return "Pressing... " + str(int(time_left)) + "s remaining"
	elif processing_queue.size() > 0:
		return "Queue: " + str(processing_queue.size()) + " items"
	else:
		return "Ready to press ingredients"
	...
	match action_id:
		"Process Item":
			if player.held_item and can_process_item(player.held_item.id):
				...
				if player.held_item:
					print("Cannot press this item: ", player.held_item.id)
					if info:
						info.show_message("This ingredient cannot be pressed.")
				else:
					if info:
...
func get_prompt() -> String:
	...
	if status != "Ready to press ingredients":
		return base_prompt + "\n" + status
	else:
		return base_prompt
