...
func create_debug_visualization():
	...
	for i in range(curve_3d.get_point_count()):
		...
		var color = Color.RED if i == 0 or i == curve_3d.get_point_count() - 1 else Color.BLUE
...
func create_path_visualization():
	"""Create a visual representation of the curve path"""
	if not curve_3d or curve_3d.get_point_count() < 2:
		return
	
	var path_viz = MeshInstance3D.new()
	path_viz.name = "Debug_Path_Visualization"
	...
	path_viz.material_override = material
