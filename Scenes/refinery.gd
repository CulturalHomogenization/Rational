...
@export var station_name: String = "Refinery"
@export var refine_time: float = 5.0

# Simple item processing dictionary: input_item -> processed_scene
@export var item_processing: Dictionary = {
	"iron": preload("res://Scenes/ChoppedCarrot.tscn"),
	"copper": preload("res://Scenes/bar.tscn")
}
...
func _ready() -> void:
	...
	processing_timer.wait_time = refine_time
...
func start_processing() -> void:
	if processing_queue.size() > 0 and not is_processing:
		...
		interaction_actions = {
			"Processing": {
				"message": "Refining " + processing_queue[0] + "...",
				"input_action": ""
			}
		}
		
		if info:
			info.show_message("Started Refining " + processing_queue[0])
...
func _on_processing_complete() -> void:
	if processing_queue.size() > 0:
		...
		if info:
			info.show_message("Refining finished!")
...
func get_processing_status() -> String:
	if is_processing:
		...
		return "Refining... " + str(int(time_left)) + "s remaining"
	elif processing_queue.size() > 0:
		return "Queue: " + str(processing_queue.size()) + " items"
	else:
		return "Ready to Refine Materials"
...
func get_prompt() -> String:
	...
	if status != "Ready to Refine ingredients":
		return base_prompt + "\n" + status
	else:
		return base_prompt
